...
extension XZComposeTextView {
    ...
    var emoticonText: String {
        ...
        attrStr.enumerateAttributes(in: NSRange(location: 0, length: attrStr.length), options: []) { (dict, range, _) in
            ...
            if let attachment = dict[NSAttributedStringKey.attachment] as? XZEmoticonAttachment {
                print("图片 \(attachment)")
...
//        print(result)
    ...
    func insertEmoticon(em: XZEmoticon?) {
        ...
        if let emoji = em.emoji,
            ...
            // UITextRange 仅用在此处！
            replace(textRang, withText: emoji)
        ...
        // 恢复光标位置，length 是选中字符的长度，插入文本之后，应该为0
